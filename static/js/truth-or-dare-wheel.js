...
function handleDeduplication() {
    ...
    const originalLines = getItemsFromInput();
    if (originalLines.length === 0) {
        resultDisplay.textContent = "Nothing to deduplicate.";
        return;
    }

    // 使用 Set 数据结构来保证唯一性，并保持原始顺序
    const uniqueLines = [...new Set(originalLines)];

    const numRemoved = originalLines.length - uniqueLines.length;

    if (numRemoved > 0) {
        itemsInput.value = uniqueLines.join('\n');
        // 调用 updateItemsArrayAndDisplay 来刷新转盘状态并保存到 localStorage
        updateItemsArrayAndDisplay('manual_update'); 
        resultDisplay.textContent = `Removed ${numRemoved} duplicate item(s).`;
    } else {
        resultDisplay.textContent = "No duplicate items found.";
    }
    playClickSound(); // 播放一个点击音效作为反馈
}

if (deduplicateButton) {
    deduplicateButton.addEventListener('click', handleDeduplication);
}
...
if (navbarToggler && navbarCollapse) {
    ...
    navLinks.forEach(link => {
        link.addEventListener('click', (event) => {
            ...
            // Only handle on-page anchor links
            if (targetSelector && targetSelector.startsWith('#')) {
                event.preventDefault(); // Prevent default browser jump

                const targetElement = document.querySelector(targetSelector);
                if (targetElement) {
                    // Hide all other toggleable sections before showing the new one
                    document.querySelectorAll('.manual-section.container[id], .use-cases-section-wrapper.container[id]').forEach(section => {
                    ...
                    // Show the target section and scroll to it
                    targetElement.style.display = 'block';
                    targetElement.scrollIntoView({ behavior: 'smooth', block: 'start' });
                }
            }

            // Also, close the mobile navbar if it's open
    ...
    // Listener to close navbar when clicking outside
